...
CREATE INDEX `eventos2_fecha` ON `kinnil`.`eventos2` (`fecha` ASC);

CREATE INDEX `eventos2_fecha_hora` ON `kinnil`.`eventos2` (`fecha` ASC, `hora` ASC);
...
CREATE TABLE IF NOT EXISTS `kinnil`.`tz` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `actual` VARCHAR(45) NULL COMMENT 'Solamente guarda el nombre del TZ actual ej \'America/Chihuahua\' esto se utiliza para guardar la hora y fecha del evento en el timezone en el que ocurrio y no en el del servidor',
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
